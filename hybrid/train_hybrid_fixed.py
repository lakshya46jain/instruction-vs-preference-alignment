...
print("="*70)
print("HYBRID TRAINING (FIXED - WITH REFERENCE MODEL)")
print("="*70)
...
# ========================================
# REFERENCE MODEL (frozen) - merged SFT
# ========================================
...
# ========================================
# POLICY MODEL (trainable) - SFT + NEW LoRA
# ========================================
print("\nLoading policy model with new LoRA layer...")
model = AutoModelForCausalLM.from_pretrained(
    BASE_MODEL, torch_dtype=torch.bfloat16, device_map="auto", trust_remote_code=True
)
# Load SFT LoRA and merge it
model = PeftModel.from_pretrained(model, SFT_CHECKPOINT, torch_dtype=torch.bfloat16)
model = model.merge_and_unload()

# Add NEW LoRA layer for DPO training
...
print(f"✓ Data: {len(dataset['train'])} train, {len(dataset['validation'])} val")
...
dpo_config = DPOConfig(
    ...
    gradient_checkpointing=False,  # CRITICAL FIX: Disabled to prevent zero gradients
    max_grad_norm=1.0, beta=0.3,
...
trainer = DPOTrainer(
    model=model,              # Policy (SFT + new LoRA, trainable)
    ref_model=ref_model,      # Reference (merged SFT, frozen)
    args=dpo_config,
    train_dataset=dataset["train"],
    eval_dataset=dataset["validation"],
    processing_class=tokenizer,
)
print("✓ Trainer ready with frozen reference")

# ========================================
# Train
# ========================================
print("\n" + "="*70)
print("TRAINING (~16 minutes)")
print("="*70)
...
print(f"✓ Saved LoRA adapters to {OUTPUT_DIR} (~10MB)")

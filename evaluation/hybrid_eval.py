...
def main():
    ...
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(
            ...
            f"Adjust DATA_PATH in hybrid_eval.py if your file lives elsewhere."
        )
    ...
    with open(RESULTS_PATH, "w", encoding="utf-8") as out_f:
        ...
        for i, example in enumerate(read_jsonl(DATA_PATH), start=1):
            ...
            hybrid_out = generate(hybrid_model, tokenizer, prompt)
            print(f"Finished HYBRID generation for example {i}")

            total += 1

            base_score, base_special = eval_instruction_following(prompt, base_out, reference)
            hybrid_score, hybrid_special = eval_instruction_following(prompt, hybrid_out, reference)

            sum_score_base += base_score
            sum_score_hybrid += hybrid_score

            if base_special and hybrid_special:
                special_examples += 1
            ...
            out_f.write("HYBRID OUTPUT:\n{}\n\n".format(hybrid_out))
            out_f.write(f"[HYBRID heuristic score: {hybrid_score:.3f}]\n\n")
        ...
        if total > 0:
            ...
            avg_hybrid = sum_score_hybrid / total
            out_f.write(f"Base   avg heuristic score (all examples): {avg_base:.3f}\n")
            out_f.write(f"Hybrid avg heuristic score (all examples): {avg_hybrid:.3f}\n")
        ...
        if special_examples > 0:
            ...
            avg_spec_hybrid = sum_spec_hybrid / special_examples
            out_f.write(
    ...
    if total > 0:
        ...
        print(f"Hybrid avg heuristic score (all examples): {avg_hybrid:.3f}")
